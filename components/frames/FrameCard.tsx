...
export default function FrameCard({ frame, priority = false }: FrameCardProps) {
  ...
  return (
    <Link
      ...
      className="masonry-item block group frame-card rounded-xl overflow-hidden bg-[var(--surface)] relative"
    ...
    >
        ...
        {frame.tags && frame.tags.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-1.5">
            {frame.tags.slice(0, 3).map((tag) => (
              <span
                key={tag}
                className="text-[10px] px-1.5 py-0.5 rounded-full bg-white/20 text-white/80"
              >
                {tag}
              </span>
            ))}
      ...
      {frame.category && (
        <div className="absolute top-2 left-2">
          <span className="text-[10px] px-2 py-0.5 rounded-full bg-black/50 text-white/80 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity">
            {frame.category.name}
          </span>
        </div>
      )}
